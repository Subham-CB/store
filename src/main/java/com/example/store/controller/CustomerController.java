...
public class CustomerController {
    ...
    public ResponseEntity<List<CustomerDTO>> getCustomers(
        ...
        final Pageable pageable = pageableBuilder.buildPageable(page,limit,sortBy,sortDir,customerSearchProps.getLimit(),
                customerSearchProps.getSortField(),customerSearchProps.getDirection());

        if (!StringUtils.hasText(name)) {
            return ResponseEntity.status(HttpStatus.OK).body(customerService.findCustomers(pageable));
        }
        else {
            return ResponseEntity.status(HttpStatus.OK).body(customerService.findCustomersNameContainingSubString(pageable,name));
        }
    }
